...
public class EvSafeServiceDataManager {
    ...
    private final CarPropertyManager.CarPropertyEventCallback carPropertyEventCallback = 
    ...
    private void handlePropertyChange(CarPropertyValue<?> value) {
        try {
            EvSafeServiceData oldData = new EvSafeServiceData(evSafeServiceData);
            int propertyId = value.getPropertyId();
            
            Log.i(TAG, String.format("Processing property change - ID: %d, Value: %s", 
            ...
            switch (propertyId) {
                ...
                case VehiclePropertyIds.GEAR_SELECTION:
                    int gearValue = ((Integer) value.getValue()).intValue();
                    updateGearStatus(gearValue);
                    break;
    
                default:
                    Log.w(TAG, String.format("Unhandled property change - ID: %d", propertyId));
                    break;
            }
    
            if (!evSafeServiceData.equals(oldData)) {
                Log.i(TAG, String.format("Data changed - updating listeners"));
                evSafeServiceDataUpdateListener.onEvSafeServiceDataUpdated(evSafeServiceData);
            } else {
                Log.d(TAG, "No data changes detected");
            }
        } catch (Exception e) {
    ...
    private float calculateEstimatedRange(float batteryPercentage) {
        return batteryPercentage *  EvSafeServiceConfigs.Defaults.MAX_RANGE / 100;
    }
